...
/** Original Players leaderboard retained unchanged as the rollback implementation. */
export default function LegacyPlayersScreen() {
  ...
  const players = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      filtered = rawPlayers.filter((player) =>
        player.name.toLowerCase().includes(q) || player.teamAbbr.toLowerCase().includes(q)
      );
    }
...
const styles = StyleSheet.create({
  ...
  segmentRow: { marginBottom: Spacing.lg },
  loadingContainer: { alignItems: 'center', paddingVertical: Spacing.xxxl, gap: Spacing.md },
  loadingText: { color: Colors.textMuted, fontSize: FontSize.md },
  emptyState: { padding: Spacing.xxxl, alignItems: 'center', gap: Spacing.xs },
  emptyText: { color: Colors.textMuted, fontSize: FontSize.md },
  emptySubtext: { color: Colors.textMuted, fontSize: FontSize.sm, textAlign: 'center', lineHeight: 18 },
  truncatedText: { color: Colors.textMuted, fontSize: FontSize.sm, textAlign: 'center', paddingVertical: Spacing.lg },
});
